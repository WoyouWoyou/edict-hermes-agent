...
BACKEND_PORT="${EDICT_BACKEND_PORT:-8000}"  # 后端端口，可通过环境变量覆盖
...
while true; do
  rotate_log
  safe_run "$SCRIPT_DIR/sync_from_hermes_runtime.py"
  ...
  if (( SCAN_COUNTER >= SCAN_INTERVAL )); then
    SCAN_COUNTER=0
    curl -s -X POST "http://127.0.0.1:${BACKEND_PORT}/api/scheduler-scan" \
