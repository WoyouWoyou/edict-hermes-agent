...
if (-not $env:HERMES_HOME) {
    $env:HERMES_HOME = Join-Path (Split-Path -Parent $ScriptDir) ".hermes"
}
...
$MaxLogSize = 10MB
$ScriptTimeout = 30
$BackendPort = $env:EDICT_BACKEND_PORT
...
while ($true) {
    ...
    Safe-Run "$ScriptDir\sync_from_hermes_runtime.py"
    ...
    if ($ScanCounter -ge $ScanInterval) {
        $ScanCounter = 0
        try {
            Invoke-RestMethod -Uri "http://127.0.0.1:$BackendPort/api/scheduler-scan" `
                -Method POST `
