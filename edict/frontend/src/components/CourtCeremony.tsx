...
export default function CourtCeremony() {
  ...
  useEffect(() => {
    const lastOpen = localStorage.getItem('hermes_court_date');
    const today = new Date().toISOString().substring(0, 10);
    const pref = JSON.parse(localStorage.getItem('hermes_court_pref') || '{"enabled":true}');
    if (!pref.enabled || lastOpen === today) return;
    localStorage.setItem('hermes_court_date', today);
    setShow(true);
