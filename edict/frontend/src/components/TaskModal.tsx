...
export default function TaskModal() {
  ...
  const hb = isDone
    ? { status: 'active' as const, label: '已完成' }
    : task.state === 'Cancelled'
      ? { status: 'unknown' as const, label: '已取消' }
      : task.heartbeat || { status: 'unknown' as const, label: '无数据' };
  ...
  const finalReply = task.output && task.output !== '-' ? task.output.trim() : '';
  ...
  const doArchive = async () => {
    try {
      const r = await api.archiveTask(task.id, true);
      if (r.ok) {
        toast('已确认并归档', 'ok');
        loadAll();
        close();
  ...
  const copyFinalReply = async () => {
    ...
    const ok = await copyText(finalReply);
    toast(ok ? '回奏已复制' : '复制失败，请从回奏正文手动选取', ok ? 'ok' : 'err');
  };
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="task-actions">
            ...
            {isDone && finalReply && (
              <button className="btn-action" style={{ background: '#2ecc8a22', color: '#2ecc8a', border: '1px solid #2ecc8a44' }} onClick={copyFinalReply}>复制回奏</button>
            )}
            {isTerminal && !task.archived && (
              <button className="btn-action" style={{ background: '#6a9eff18', color: '#8fb6ff', border: '1px solid #6a9eff44' }} onClick={doArchive}>确认归档</button>
            )}
          </div>
          ...
          {/* Output */}
          {finalReply && (
            <div className="m-section reply-section">
              <div className="reply-head">
                <div>
                  <div className="m-sec-label">回奏正文</div>
                  <div className="reply-sub">这是 Hermes agent 写回 Dashboard 的最终回复，不会自动发送到外部渠道。</div>
                </div>
                <button className="reply-copy" onClick={copyFinalReply}>复制</button>
              </div>
              <pre className="reply-box">{finalReply}</pre>
            </div>
          )}

          {/* Live Activity */}
          <LiveActivitySection data={activityData} isDone={isTerminal} logRef={logRef} />
        </div>
      </div>
    </div>
  );
