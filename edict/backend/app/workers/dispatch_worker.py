"""Dispatch Worker — 消费 task.dispatch 事件，执行 Hermes Agent profile 调用。
...
3. 调用 Hermes CLI: `hermes --profile xxx chat --quiet --source edict -q "..."`
...
def _resolve_agents_dir() -> pathlib.Path:
    ...
    if settings.hermes_project_dir:
        return pathlib.Path(settings.hermes_project_dir) / "agents"
...
def _build_reminder(agent_id: str, payload: dict) -> str:
    ...
    if not reminders:
        reminders.append("本轮是轻量派发：优先给出阶段性结论和下一步建议，避免长时间探索。")
...
def _resolve_project_root() -> pathlib.Path:
    ...
    settings = get_settings()
    if settings.hermes_project_dir:
        return pathlib.Path(settings.hermes_project_dir)
...
def _next_state_after_success(current: TaskState, assignee_org: str | None) -> TaskState | None:
    """Hermes 未主动更新看板时的保守自动推进。"""
    if current == TaskState.Taizi:
        return TaskState.Zhongshu
    if current == TaskState.Zhongshu:
        return TaskState.Menxia
    if current == TaskState.Menxia:
        return TaskState.Assigned
    if current == TaskState.Assigned:
        return TaskState.Doing if assignee_org in ORG_AGENT_MAP else TaskState.Blocked
    if current in {TaskState.Doing, TaskState.Next}:
        return TaskState.Review
    if current in {TaskState.Review, TaskState.PendingConfirm}:
...
class DispatchWorker:
    ...
    async def _recover_pending(self):
        events = await self.bus.claim_stale(
            TOPIC_TASK_DISPATCH, GROUP, CONSUMER, min_idle_ms=0, count=20
        )
        if events:
    ...
    async def _call_hermes(
        ...
        dispatch_toolsets = os.environ.get("DISPATCH_HERMES_TOOLSETS") or settings.hermes_toolsets
        if dispatch_toolsets:
            cmd.extend(["--toolsets", dispatch_toolsets])
        ...
        def _run():
            try:
                proc = subprocess.run(
                    ...
                    env=env,
                    cwd=settings.hermes_project_dir or None,
                )
                return {
                ...
                return {"returncode": -1, "stdout": "", "stderr": "agent runtime command not found"}
            finally:
                # 清理临时上下文文件
                if context_file:
                    try:
    ...
    async def _hydrate_payload(self, task_id: str, payload: dict) -> dict:
        """Merge the lightweight event payload with the current DB task snapshot."""
        try:
    ...
    async def _record_agent_success(self, task_id: str, agent: str, stdout: str) -> None:
        ...
        try:
            ...
            log.warning(f"Cannot persist agent output for invalid task id: {task_id}")
            return

        content = stdout.strip() or "Agent completed without textual output."
        async with async_session() as session:
            svc = TaskService(session)
            task = await svc.add_progress(task_uuid, agent, content[:8000])
            current = task.state if isinstance(task.state, TaskState) else TaskState(str(task.state))
            next_state = _next_state_after_success(current, task.assignee_org)
            if not next_state:
                return
            try:
