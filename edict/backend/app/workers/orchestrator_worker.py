...
class OrchestratorWorker:
    ...
    async def _on_task_status(self, event_type: str, payload: dict, trace_id: str):
        ...
        # 如果进入 assigned/doing 状态，需要查找六部对应 agent
        if new_state == TaskState.Assigned:
            ...
            if org:
                ...
                agent = "shangshu"
        elif new_state in {TaskState.Doing, TaskState.Next}:
            org = payload.get("assignee_org", "")
            if org:
                agent = ORG_AGENT_MAP.get(org)
            if not agent:
                log.warning(f"Task {task_id} entered {new_state.value} without a valid assignee_org")
                return
