...
# ────────────────────────────────────────────────────────────────
# Helper: patch BASE and HERMES_HOME to isolate tests
# ────────────────────────────────────────────────────────────────
...
def project(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sac, 'AGENT_ORDER', ['aaa'])
...
class TestSyncScriptsToWorkspaces:
    ...
    def test_creates_symlinks_in_workspace(self, project):
        ...
        ws = project.home / '.hermes' / 'profiles' / 'aaa' / 'scripts'
        assert ws.is_dir()
    ...
    def test_no_self_referential_symlinks_when_workspace_scripts_is_dir_symlink(
        ...
        profile = project.home / '.hermes' / 'profiles' / 'aaa'
        profile.mkdir(parents=True, exist_ok=True)
        profile_scripts = profile / 'scripts'
        os.symlink(project.scripts, profile_scripts)  # directory-level symlink
